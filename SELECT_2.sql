...
SELECT COUNT(DISTINCT track_name), year_of_realise FROM track t
	JOIN album a ON t.album_id = a.id
    WHERE year_of_realise BETWEEN '2019-01-01' AND '2021-01-01'
    GROUP BY a.year_of_realise;
...
SELECT names, year_of_realise FROM musician m
...
SELECT DISTINCT title_collection, names FROM  collection c
    JOIN track t ON c.id = t.album_id
	   JOIN musician m ON m.id = t.album_id
    WHERE names = 'Taylor Swift'; 

-- название альбомов, в которых присутствуют исполнители более 1 жанра;

SELECT title_album, COUNT (genre_id) FROM album a
    JOIN genres_musicians gm ON a.id = gm.musician_id
    GROUP BY a.title_album
    HAVING COUNT(genre_id) > 1;

-- наименование треков, которые не входят в сборники;

SELECT track_name, COUNT(track_id) FROM track t
...
SELECT title_album, COUNT (album_id) FROM album a
    JOIN track t ON a.id = t.album_id
    GROUP BY a.title_album
    HAVING COUNT(album_id) = (SELECT MIN(album_id) FROM track);
